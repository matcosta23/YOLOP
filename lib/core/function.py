...
def validate_with_speedup_engine(epoch,config, val_loader, val_dataset, model, engine, criterion, 
            output_dir, tb_log_dir, writer_dict=None, logger=None, device='cpu', rank=-1):
    """
    Function clearly inpired in the 'validate' one. However, adapted for
    using NNI framework's engine instead of the PyTorch model.
    """
    # setting
    max_stride = 32
    weights = None
    ...
    # NOTE : Since we'll do the inference with the engine, we don't have to set the model to evaluation mode.
    #model.eval()
    jdict, stats, ap, ap_class, wandb_images = [], [], [], [], []
    ...
    for batch_i, (img, target, paths, shapes) in tqdm(enumerate(val_loader), total=len(val_loader)):
        ...
        with torch.no_grad():
            ...
            t = time_synchronized()
            # NOTE : Inference with the engine.
            inference_outputs, engine_time = engine.inference(img)
            det_out, da_seg_out, ll_seg_out = inference_outputs
            t_inf = time_synchronized() - t
            if batch_i > 0:
                T_inf.update(t_inf/img.size(0),img.size(0))
            ...
            # TODO : Verify if criterion can be calculated with model even inference done by the engine.
            total_loss, head_losses = criterion((train_out,da_seg_out, ll_seg_out), target, shapes, model)   #Compute loss
            losses.update(total_loss.item(), img.size(0))
